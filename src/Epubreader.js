...
3. Third item`);
    ...
    const handlers = useSwipeable({
        onSwipedLeft: () => nextPiece(),
        onSwipedRight: () => prevPiece()
    });
  // Load the first piece of text
  useEffect(() => {
        const handleKeyPress = (event) => {
            if (event.key === 'ArrowLeft') {
              console.log(chapno, index, nxtchapno, nxtind, bookname);
              prevPiece();
    ...
    }, [chapno, index, nxtchapno, nxtind, prevPiece, nextPiece]);
  useEffect(()=>{
  ...
  return (
    <div className="epub">
      <div className="selectionDiv">
        ...
        <select name="selection" className="selection" value={bookname} onChange={(i)=>{ console.log("Changed!");  chapno=0; index=0;bookname=i.target.value; setBookname(bookname);}}>
      ...
      <div className="main">
        ...
        <div className="MainBook">
          <div {...handlers} className="text">
            <ReactMarkdown remarkPlugins={[remarkGfm]}          // Enables tables
            rehypePlugins={[rehypeKatex]}>{text}</ReactMarkdown>
          </div>
        </div>
        <div className="buttonsTray">
